...
verify_imported_images() {
  local host_ip="$1"
  local host_name="$2"
  printf '[CHECK] StorageClass images on %s\n' "${host_name}"
  remote_cmd "${host_ip}" "sudo bash -lc '
set -euo pipefail
images=\$(ctr -n k8s.io images list)
grep -q \"rancher/local-path-provisioner\" <<<\"\$images\"
grep -q \"busybox\" <<<\"\$images\"
'"
  printf '[OK] StorageClass images present on %s\n' "${host_name}"
}

verify_imported_images "${AIRGAP_MASTER_PRIVATE_IP}" "${AIRGAP_MASTER_HOST}"
verify_imported_images "${AIRGAP_WORKER1_PRIVATE_IP}" "${AIRGAP_WORKER1_HOST}"
