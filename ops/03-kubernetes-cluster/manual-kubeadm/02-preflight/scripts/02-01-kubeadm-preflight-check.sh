...
# This script is copied to each node by the manual kubeadm preflight step.
ROLE="control-plane"
PEERS=()
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Usage:
  ./02-01-kubeadm-preflight-check.sh [--role control-plane|worker] [--peer <ip-or-host>]...

Examples:
  ./02-01-kubeadm-preflight-check.sh --role control-plane
  ./02-01-kubeadm-preflight-check.sh --role worker --peer 192.168.56.10 --peer 192.168.56.11
EOF
      exit 0
      ;;
    *)
...
check_ports() {
  ...
  if [[ -f /etc/kubernetes/admin.conf || -f /etc/kubernetes/kubelet.conf ]]; then
    pass "Kubernetes config exists; skipping pre-install port availability check"
    warn "Ports may already be in use because kubeadm has completed on this node"
    return
  fi
