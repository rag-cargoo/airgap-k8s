...
verify_containerd() {
  local host_ip="$1"
  local host_name="$2"
  printf '[CHECK] containerd on %s\n' "${host_name}"
  remote_cmd "${host_ip}" "sudo bash -lc '
set -euo pipefail
rpm -q containerd runc >/dev/null
systemctl is-active --quiet containerd
grep -q \"SystemdCgroup = true\" /etc/containerd/config.toml
'"
  printf '[OK] containerd verified on %s\n' "${host_name}"
}

verify_containerd "${AIRGAP_MASTER_PRIVATE_IP}" "${AIRGAP_MASTER_HOST}"
verify_containerd "${AIRGAP_WORKER1_PRIVATE_IP}" "${AIRGAP_WORKER1_HOST}"
