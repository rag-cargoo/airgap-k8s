...
apply_node_baseline() {
  local host_ip="$1"
  local host_name="$2"
  printf '[STEP] apply node baseline on %s\n' "${host_name}"
...
apply_node_baseline "${AIRGAP_MASTER_PRIVATE_IP}" "${AIRGAP_MASTER_HOST}"
apply_node_baseline "${AIRGAP_WORKER1_PRIVATE_IP}" "${AIRGAP_WORKER1_HOST}"
