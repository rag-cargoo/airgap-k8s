...
verify_node_baseline() {
  local host_ip="$1"
  local host_name="$2"
  printf '[CHECK] node baseline on %s\n' "${host_name}"
...
! swapon --show | tail -n +2 | grep -q .
'"
  printf '[OK] node baseline verified on %s\n' "${host_name}"
}

verify_node_baseline "${AIRGAP_MASTER_PRIVATE_IP}" "${AIRGAP_MASTER_HOST}"
verify_node_baseline "${AIRGAP_WORKER1_PRIVATE_IP}" "${AIRGAP_WORKER1_HOST}"
