...
install_kubernetes_packages() {
  local host_ip="$1"
  local host_name="$2"
  printf '[STEP] install Kubernetes packages on %s\n' "${host_name}"
  remote_cmd "${host_ip}" "sudo bash -lc '
set -euo pipefail
if ! rpm -q kubeadm kubelet kubectl kubernetes-cni cri-tools >/dev/null 2>&1; then
  rpm_files=\$(find ${SERVER_ASSETS_DIR}/kubernetes/packages/kubernetes -maxdepth 1 -type f -name \"*.rpm\" ! -name \"amazon-linux-repo-cdn-*\" | sort)
  test -n \"\${rpm_files}\"
...
install_kubernetes_packages "${AIRGAP_MASTER_PRIVATE_IP}" "${AIRGAP_MASTER_HOST}"
install_kubernetes_packages "${AIRGAP_WORKER1_PRIVATE_IP}" "${AIRGAP_WORKER1_HOST}"
