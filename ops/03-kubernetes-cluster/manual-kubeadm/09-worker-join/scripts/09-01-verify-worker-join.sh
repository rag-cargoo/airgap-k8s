...
printf '[CHECK] worker node joined\n'
remote_cmd "${AIRGAP_WORKER1_PRIVATE_IP}" "sudo test -f /etc/kubernetes/kubelet.conf"
remote_cmd "${AIRGAP_MASTER_PRIVATE_IP}" "sudo bash -lc '
set -euo pipefail
KUBECONFIG=/etc/kubernetes/admin.conf kubectl get node ${AIRGAP_WORKER1_HOST} >/dev/null
'"
